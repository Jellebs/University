...
filsti = "../Profiler"
luftFilSti = filsti + "/luft_500middel.pulse.csv"
refFilSti = filsti + "/ren_silicium525_1_scan1.pulse.csv"

filename = '16_26'; % Define the base filename (without extensions)
samFilSti = filsti + "/proeve" + filename + "_scan5.pulse.csv"

PurgeAir1 = importfileT2(luftFilSti); % PurgeAir1(:, 2) = PurgeAir1(:, 2) * 1e-9
PurgeRef = importfileT2(refFilSti); % PurgeRef(:, 2) = PurgeRef(:, 2) * 1e-9
PurgeAir = importfileT2(luftFilSti); % PurgeAir(:, 2) = PurgeAir(:, 2) * 1e-9
PurgeSam = importfileT2(samFilSti); % PurgeSam(:, 2) = PurgeSam(:, 2) * 1e-9
...
d2= 501.3; %µm,     thickness of the SAMPLE substrate
d = 5.5e-6; % Thin-film thickness in meters (10 nm example)
...
plot(PurgeSam(:,1),PurgeAir1(:,2), 'b','LineWidth',2)
...
hold on 
plot(freq, PhaseConja)
plot(freq, PhaseConj1)
plot(freq, PhaseConj2)
plot(freq, PhaseConja1)
hold off
...
xlim(x_limit); box on;
% legend('Reference')
 save(['new' filename '.mat'], 'freq', 'n', 'n1', 'n_f_eq_5_6', 'zeta')
